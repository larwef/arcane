...
var (
	privateKey1 = parsePrivateKey("test/data/key1")
	privateKey2 = parsePrivateKey("test/data/key2")
	privateKey3 = parsePrivateKey("test/data/key3")

	publicKey1 = parsePublicKey("test/data/key1.pub")
	publicKey2 = parsePublicKey("test/data/key2.pub")
	publicKey3 = parsePublicKey("test/data/key3.pub")
)
...
func TestSealerAndOpener(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "Simple test",
			sealer:      &Sealer{privateKey1, publicKey2},
			opener:      &Opener{privateKey2},
...
func parsePublicKey(path string) *rsa.PublicKey {
	...
	if err != nil {
		log.Fatalf("Unable to get public key %q for test: %v", path, err)
	}
